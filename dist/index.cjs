...
const MonnifyButton = ({ text, className, children, disabled, type = 'button', onSuccess, onClose, ...others }) => {
    ...
    return (React__default.default.createElement("button", { type: type, className: className, disabled: disabled, onClick: () => initializePayment(onSuccess, onClose) }, text || children));
};

const MonnifyContext = React.createContext({
...
const MonnifyProvider = ({ children, onSuccess, onClose, ...others }) => {
    const initializePayment = useMonnifyPayment(others);
    return (React__default.default.createElement(MonnifyContext.Provider, { value: { initializePayment, onSuccess, onClose } }, children));
};
...
const MonnifyConsumerChild = ({ children, forwardedRef, }) => {
    const { initializePayment, onSuccess, onClose } = React.useContext(MonnifyContext);
...
const MonnifyConsumer = React.forwardRef(({ children, onSuccess: paraSuccess, onClose: paraClose, ...others }, ref) => {
    const onSuccess = paraSuccess ? paraSuccess : () => null;
    const onClose = paraClose ? paraClose : () => null;
    return (React__default.default.createElement(MonnifyProvider, { ...others, onSuccess: onSuccess, onClose: onClose },
        React__default.default.createElement(MonnifyConsumerChild, { forwardedRef: ref }, children)));
});
MonnifyConsumer.displayName = 'MonnifyConsumer';

exports.MONNIFY_PRODUCTION_SDK_URL = MONNIFY_PRODUCTION_SDK_URL;
exports.MONNIFY_SANDBOX_SDK_URL = MONNIFY_SANDBOX_SDK_URL;
exports.MonnifyButton = MonnifyButton;
exports.MonnifyConsumer = MonnifyConsumer;
exports.MonnifyProvider = MonnifyProvider;
exports.getMonnifyScriptUrl = getMonnifyScriptUrl;
exports.useMonnifyPayment = useMonnifyPayment;
exports.useMonnifyScript = useMonnifyScript;
